...
from models.sphere_model import Generator, Discriminator
    ...
    if data_count == opt.n_epochs:
        ...
        for epoch in range(data_count, opt.n_epochs):
            for i, (imgs, _) in enumerate(train_loader):
                ...
                fake_loss = criterion(discriminator(real_imgs)) - criterion(discriminator(gen_imgs))

                # Gradient penalty
                # gradient_penalty = lambda_gp * compute_gradient_penalty_sphere(discriminator, real_imgs.data, gen_imgs.data)
                d_loss = fake_loss
                d_loss.backward()
